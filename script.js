/* =============================================
   KIANGGEH COLD-CHAIN TRANSPORT DOCK
...
(function initHeroScene() {"
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const color = new THREE.Color(Math.random() > 0.4 ? 0xffffff : 0x38bdf8);
    colors[i3] = color.r;
    colors[i3 + 1] = color.g;
    colors[i3 + 2] = color.b;
  ...
  // --- CUSTOM THREE.JS CUSTOM GEOMETRY INJECTION ---
    ...
    // Floating boxes representing cubes of cold chain seafood ice blocks
    function createIceBlock(color) {
      ...
      const mat = new THREE.MeshStandardMaterial({
        ...
        metalness: 0.1,
        transparent: true,
        opacity: 0.65, // translucent ice look
        flatShading: true
      });
    ...
    for (let i = 0; i < 10; i++) {
      ...
      iceCubes.push({
        mesh: cube,
        rotSpeed: { x: (Math.random() - 0.5) * 0.015, y: (Math.random() - 0.5) * 0.015, z: 0 },
  ...
  const dirLight2 = new THREE.DirectionalLight(0x1e3a8a, 0.8);
  dirLight2.position.set(-10, -5, -5);
  scene.add(dirLight2);

  const pointLight = new THREE.PointLight(0x38bdf8, 1.0, 45);
